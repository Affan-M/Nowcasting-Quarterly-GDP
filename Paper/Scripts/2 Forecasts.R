...
df <-
  df_x[["Data"]] |>
  filter(date >= "Jan 2004" & !id %in% c(249, 250, 251, 252, 10000)) |>
...
# Adding in EIA oil prices
data_list[["x_tidy"]] <-
  bind_rows(
    data_list[["x_tidy"]],
    df_x[["Data"]] |>
      filter(id == 10000 & date >= "Jan 2004" & date <= max_month)
  )
...
for (y in y_vec_names) {
  ...
  if (!is_empty(names(model_stack$member_fits))) {
    ...
    stack_issues <- c(stack_issues, glue("{y}"))
  }
...
for (dfm in dfm_vec) {
  ...
  for (y_i in y_vec) {
    ...
    final_vec <- head(agg_mat_list[["y"]][, y_i], 1) |> as.numeric()
...
reconciled_fc <-
  reconcile(
    r_forecast_list = forecast_list,
    r_fitted_list = fitted_list
  )


# Tidying forecast list and exporting it to dataframe ---------------------
...
best_model_vec <-
  list.files("Best models/") |> str_remove_all(".parquet")

for (i in best_model_vec) {
  best_model_list[[i]] <-
    read_parquet(file = glue("Best models/{i}.parquet"))

  # Checking the windows and keeping only the most recent window's best models
  # to reconcile the forecasts, in the case where best model was selected by
  # looking back a specific number of windows
  windows <-
    best_model_list[[i]] |>
    distinct(window) |>
    pull()
  ...
  if (length(windows) > 1) {
    latest_window <- windows |> max()

    best_model_list[[i]] <-
      best_model_list[[i]] |>
      filter(window == latest_window) |>
...
save(list = ls(), file = "Forecasts.rdata")

load("Forecasts.rdata")

write_parquet(
  x = forecast_df,
  sink = "forecasts.parquet"
)
...
print(glue("Time for forecasts: {format(forecast_time)}"))



# Analysis: Level and growth numbers --------------------------------------

order <- df_y[["Meta"]] |> pull(name)

comb <- "Combination_3 Rec_BU"
...
annual_table_df <-
  ...
  mutate(
    ...
    growth = amount/dplyr::lag(amount) * 100 - 100
  )
...
print(annual_table_level)
print(annual_table_growth)


# Quarterly
H <- (as.yearqtr(glue("{year(max_y_date)} Q4")) - max_y_date) * 4
...
quarterly_table_df <-
  ...
  mutate(
    amount = amount/1000000,
    growth = amount/dplyr::lag(amount, 4) * 100 - 100
  ) |>
  filter(date >= "2021 Q1")

quarterly_table_level <-
  quarterly_table_df |>
  pivot_wider(
    id_cols = name,
    names_from = date,
...
print(quarterly_table_level)
print(quarterly_table_growth)

list(
  "Qtr_Levels" = quarterly_table_level,
  "Qtr_Growth" = quarterly_table_growth,
  "Ann_Levels" = annual_table_level,
  "Ann_Growth" = annual_table_growth
) |>
  write.xlsx(file = "Nowcasts.xlsx")


# Analysis: Important variables -------------------------------------------

selected_best_mod <- best_model_list[[word(comb, 1)]]

if (str_detect(word(comb, 2), "BU")) {
  selected_best_mod <-
    selected_best_mod |>
    filter(
...
model_list[["GDP"]] <-
  tibble(
    "Series" = "GDP",
    "Model" = "Bottom up reconciliation",
    "Variable" = ""
  )

model_list[["Taxes_less_subsidies"]] <-
  tibble(
    "Series" = "Taxes less subsidies",
    "Model" = "Bridge_levels",
    "Variable" = "Bednights"
  )

fisheries_vip <-
  tidymodels$XGBoost$`Fisheries industry`$VIP |>
  mutate(
    Variable = str_replace_all(string = Variable, pattern = "[:punctuation:]", " ") |>
      str_to_sentence(string = _)
  )
...
model_list[["Fisheries"]] <-
  tibble(
    "Series" = "Fisheries",
    "Model" = "XGBoost",
    "Variable" = fisheries_vip_vec
  )
...
model_list[["Trade"]] <-
  tibble(
    "Series" = "Trade",
    "Model" = "Bridge_levels",
    "Variable" = "Total imports - CPI deflated"
  )

model_list[["Tourism"]] <-
  tibble(
    "Series" = "Tourism",
    "Model" = "Bridge_ARIMA_levels",
    "Variable" = "Bednights"
  )

transportcomm_vip <-
  tidymodels$Lasso$`Transportation and communication industry`$VIP |>
  mutate(
    Variable = str_replace_all(string = Variable, pattern = "[:punctuation:]", " ") |>
      str_to_sentence(string = _)
  )
...
model_list[["Transport_communication"]] <-
  tibble(
    "Series" = "Transport and communication",
    "Model" = "Lasso",
    "Variable" = transportcomm_vip
  )
...
model_list[["Financial"]] <-
  tibble(
    "Series" = "Financial",
    "Model" = "Bridge_ARIMA_levels",
    "Variable" = "Loans and advances, deposits (5SG) - Avg"
  )

model_list[["Construction_realestate"]] <-
  tibble(
    "Series" = "Construction and real estate",
    "Model" = "Bridge_ARIMA_levels",
    "Variable" = "Construction-related imports - PINDU deflated"
  )

model_list[["Publicadmin_health_education"]] <-
  tibble(
    "Series" = "Public admin, health and education",
    "Model" = "AutoARIMA",
    "Variable" = "Past trends and mean"
  )

miscellaneous_vip <-
  tidymodels$ElasticNet$Miscellaneous$VIP |>
  mutate(
    Variable = str_replace_all(string = Variable, pattern = "[:punctuation:]", " ") |>
      str_to_sentence(string = _)
  )

miscellaneous_vip_vec <-
  miscellaneous_vip |>
  head(5) |>
  pull(Variable) |>
  paste(collapse = ", ")

model_list[["Miscellaneous"]] <-
  tibble(
    "Series" = "Miscellaneous",
    "Model" = "ElasticNet",
    "Variable" = miscellaneous_vip_vec
  )

construction_related_imports_definition <-
  df_x[["Meta"]] |>
  filter(id %in% c(3501, 3502, 3509, 3513, 3514, 3525)) |>
  mutate(name = str_remove_all(string = name, pattern = "^imports_") |>
           str_replace_all(string = _, pattern = "[:punctuation:]", " ") |>
           str_to_sentence(string = _))

model_vip_df <- bind_rows(model_list)
...
series <-
  c(3509, # Trade - wood, metal, cement and aggregates,
    3513, # Trade - machinery, mechanical appliances
    3514, # Trade - electrical, electronic machinery
    3486, # Trade - Total import
    374,  # Level 2 housing water electricity gas and other fuels,
    4066) # Deflator

query_data_inputs <-
  query_db(series, frequency = "Annual", include_incomplete = FALSE)

order_data_inputs <-
  query_data_inputs[["Meta"]] |>
  select(id, name) |>
  arrange(match(id, series)) |>
  pull(name)

order_data_inputs <- c(order_data_inputs, "crude_oil_prices")

data_inputs <-
  bind_rows(
    query_data_inputs[["Data"]],
    df_x[["Data"]] |>
      filter(id == 10000) |>
  ...
  pivot_wider(id_cols = date, names_from = name, values_from = amount) |>
  select(date, all_of(order_data_inputs)) |>
  filter(date >= 2003) |>
  rename(deflator = "Real GDP at market price")


# Excel list
list(
  "Annual_levels" = annual_table_level,
  "Annual_growth" = annual_table_growth,
  "Quarterly_levels" = quarterly_table_level,
  "Quarterly_growth" = quarterly_table_growth,
  "Model_VIP" = model_vip_df,
  "GDP_inputs" = data_inputs
) |>
  write.xlsx(
    file = "Nowcasting_tables.xlsx"
  )
...
check_df <-
  agg_data_list[["x_tidy"]] |>
  filter(id %in% c(10004, 10005, 40001)) |>
  pivot_wider(id_cols = date, names_from = name, values_from = amount) |>
  mutate(across(.cols = !date, .fns = function(x) x / dplyr::lag(x) * 100 - 100))
...
tnc_val <-
  ...
  filter(
    date == "2021 Q4",
    name == "Transportation and communication industry"
  ) |>
  pull(amount)
...
forecast_df |>
  filter(
    date == "2022 Q4",
    series == "Transportation and communication industry"
  ) |>
  mutate(growth = forecast/tnc_val * 100 - 100) |>
  arrange(growth) |> View()
...
wrt_val <-
  ...
  filter(
    date == "2021 Q4",
    name == "Wholesale and retail trade industry"
  ) |>
  pull(amount)
...
forecast_df |>
  filter(
    date == "2022 Q4",
    series == "Wholesale and retail trade industry"
  ) |>
  mutate(growth = forecast/wrt_val * 100 - 100) |>
  arrange(growth) |> View()
...
forecast_df |>
  filter(
    date == "2022 Q4",
    series == "Fisheries industry"
  ) |>
  mutate(growth = forecast/fish_val * 100 - 100) |>
  arrange(growth) |> View()
